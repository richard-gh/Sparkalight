...
def SearchPerson(request):
    ...
    if request.GET.get('text'):
        ...
        return render(request,'searchs.html',{'form':form,"posts":posts,'formula': request.session.get('text'),'person':person})
...
@page_template('pagination.html')
@login_required
def Myfriend(request,template='friend.html', extra_context=None):
    context = {
...
def friendship_accept(request, friendship_request_id):
    ...
    if f_request.to_user == request.user and f_request.to_user.is_active == True:
        f_request.accept()
        return HttpResponseRedirect(reverse('world:Myfriend'))

    HttpResponseRedirect(reverse('world:Myfriend'))
...
def friendship_cancel(request, username):
    try:
        ...
        return HttpResponseRedirect(reverse('world:EditFriend'))
    user = Person.objects.get(username=request.user)
...
def follower_remove(request, followee_username):
    ...
    return HttpResponseRedirect(reverse('world:FollowingAll'))
...
def PetRegistration(request):	
    ...
    if request.method =='POST':
        ...
        if form.is_valid():
            ...
            username = Person.objects.get(username=request.user)
...
def PictureCreator(request):
	...
	if request.method == "POST":
		...
		if form.is_valid():
			...
			picture.save()
			return HttpResponseRedirect(reverse('world:PictureCreator'))
...
def BoardEditor(request):
        ...
        form = BoardDescriptionForm(request.POST,)
	...
	if request.POST.has_key('process'):
            ...
            if request.POST['process'] == 'third':
                if form.is_valid():
                        ...
                        try:
                            ...
                            bo.name = boardname
                            bo.save()
                            return HttpResponseRedirect(reverse('world:BoardEditor'))
                        except Board.DoesNotExist:
                            return HttpResponseRedirect(reverse('world:BoardEditor'))
                return HttpResponseRedirect(reverse('world:BoardEditor'))
                ...
                if formsss.is_valid():
                    ...
                    try:
                        ...
                        return render(request,'boardeditor.html',{'b':b,'form':form,'board':BoardNameForm(request.user),'BoardChangeNameForm':BoardChangeNameForm(request.user),'BoardPrivateForm':BoardPrivateForm(request.user),'person':person})
                ...
                if formssss.is_valid():
                    ...
                    try:
                        ...
                        return render(request,'boardeditor.html',{'c':c,'forms':forms,'board':BoardNameForm(request.user),'BoardChangeNameForm':BoardChangeNameForm(request.user),'BoardPrivateForm':BoardPrivateForm(request.user),'person':person})
        ...
        return render(request,'boardeditor.html',{'board':board,'BoardChangeNameForm':BoardChangeName,'BoardPrivateForm':BoardPrivate,'person':person})

              

@login_required
def ChangePicture(request,picture_id):
    picture = picture_id
    try:
...
@login_required
def CancelFriend(request,slug):
    username = Person.objects.get(slug=slug)
    user = User.objects.get(username=username)
    user1 = User.objects.get(username=request.user)
    try:
        friend = Friendship.objects.filter(user=user,friend=user1)
        friend.delete()
        return HttpResponseRedirect(reverse('world:Myfriend'))
...
def AddFriend(request,slug):
    ...
    try:
        friend = Friendship.objects.get(user=user1,friend=user2)
        if friend:
            friend1 = Friendship.objects.create(user=user2,friend=user1,is_accepted=True)
            friend = Friendship.objects.get(user=user1,friend=user2)
...
@login_required
def FollowingAll(request,template='editfollowingall.html', extra_context=None):
    ...
    extra_context = {'EditFollowing':EditFollowing}
    if extra_context is not None:
        context.update(extra_context)
...
@page_template('page.html')
@login_required
def EditFriend(request,template='editfriend.html', extra_context=None):
    ...
    extra_context = {'EditFriend':EditFriend,}
    if extra_context is not None:
        context.update(extra_context)
...
@login_required
def friendstar(request,slug):
    try:
        person = Person.objects.get(slug=slug)
    except Person.DoesNotExist:
            return HttpResponseRedirect(reverse('world:EditFriend'))
    user = User.objects.get(username=request.user)
    if Friendship.objects.get(user=user,friend=person.user) and Friendship.objects.get(user=person.user,friend=user):
        fried1 = Friendship.objects.get(user=user,friend=person.user)
        fried2 =Friendship.objects.get(user=person.user,friend=user)
        fried1.delete()
        fried2.delete()
        return HttpResponseRedirect(reverse('world:EditFriend'))
    else:
	...
	if request.user.is_authenticated():
		if request.is_ajax():
			try:
				...
				body = body.strip()
				if not body:
					return HttpResponse(json.dumps({'success':'false', "msg":"Comment can not be blank."}), content_type="application/json")
				car_obj = Board.objects.get(pk=car_id)			
				if car_obj.comments == False:
					return HttpResponse(json.dumps({'success':'false', "msg":"Invalid request"}), content_type="application/json")
				if car_obj.friends == True:
                                    if not Friend.objects.are_friends(request.user, car_obj.user) or board.user == request.user:
                                        return HttpResponse(json.dumps({'success':'false', "msg":"Invalid request"}), content_type="application/json") 
				review_obj = BoardComment.objects.create(user=request.user,body=body,board=car_obj)
				
				return HttpResponse(json.dumps({'success':'true', "msg":"Success", "user" : request.user.username}), content_type="application/json")
			except Exception,e:
				return HttpResponse(json.dumps({'success':'false', "msg":"Some internal error occured request could not be completed."}), content_type="application/json")
			return HttpResponse(json.dumps({'success': 'true', 'count': like_count}), content_type="application/json")
	else:
		return HttpResponse(json.dumps({'success': 'false', "msg":"Your session has expired. Please login to post a comment."}), content_type="application/json")
...
def delete_comment(request):
	if request.user.is_authenticated():
		...
		return HttpResponse(json.dumps({'success': 'false', "msg":"Your session has expired. Please login again."}), content_type="application/json")
			...
			try:
				...
				if search_key:
					if shirt_size == "all":
						...
						board_count = Board.objects.filter(name__icontains=search_key, shirt_size=shirt_size, id__in=board_ids).count()
						
					if page_num in (0,1):
						if shirt_size == "all":
							boards = Board.objects.filter(name__icontains=search_key, id__in=board_ids)[:per_page]
						else:
							boards = Board.objects.filter(name__icontains=search_key, shirt_size=shirt_size, id__in=board_ids)[:per_page]
					else:
						if shirt_size == "all":
							boards = Board.objects.filter(name__icontains=search_key, id__in=board_ids)[start_index:end_index]
						else:
							boards = Board.objects.filter(name__icontains=search_key, shirt_size=shirt_size, id__in=board_ids)[start_index:end_index]
				...
				## Start: Block for Most Recent or Most Liked board search ##			
					...
					if page_num in (0,1):
						if shirt_size == "all":
							...
							if most_recent:
								boards = Board.objects.filter(shirt_size=shirt_size, id__in=board_ids).order_by("-created")[:per_page]
							else:
						...
						if shirt_size == "all":
							if most_recent:
								boards = Board.objects.filter(id__in=board_ids).order_by("-created")[start_index:end_index]
							elif most_liked:
								b = Board.objects.filter(id__in=board_ids)
								boards = b.annotate(num_likes=Count('likeboard')).order_by('-num_likes')[start_index:end_index]
						else:
							board_count = Board.objects.filter(shirt_size=shirt_size, id__in=board_ids).count()
							if most_recent:
								boards = Board.objects.filter(shirt_size=shirt_size, id__in=board_ids).order_by("-created")[start_index:end_index]
							else:
								b = Board.objects.filter(shirt_size=shirt_size, id__in=board_ids)
								boards = b.annotate(num_likes=Count('likeboard')).order_by('-num_likes')[start_index:end_index]
