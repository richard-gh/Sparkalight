...
@login_required
def message(request):    
    posts = Message.objects.filter(recipient=request.user,sentmessage=True,is_deleted_by_recipient=False).order_by("-created")  
...
def readsentmessage(request,id):
    ...
    if request.method =='POST':
        ...
        if form.is_valid():
            ...
            Message.objects.create(user=request.user,recipient=messages.recipient,body=body,thread=messages.thread,sentmessage=True,read=False)
...
def Create(request):
    ...
    if request.POST.get('_send', False):
        ...
        if form.is_valid():
            ...
            message = form.cleaned_data['message']
            thread = Thread.objects.create(subject=subject,user=request.user)
            Message.objects.create(user=request.user,recipient=request.user,body=message,thread=thread,draft=True,sentmessage=False)
            return HttpResponseRedirect(reverse('world:message'))
